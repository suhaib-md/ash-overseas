...
| Backups    | D1 **Time Travel** (always-on, 30-day point-in-time recovery, no cost) **plus** a scheduled Worker/Workflow exporting a full SQL dump to **R2** for long-term retention.                              | Satisfies NFR-B1/B2/B3. Restore must be _performed and verified_ before handoff.                                                                                                                                                                                                                               |
...
**3.2 Backups & verified restore** — confirm D1 Time Travel; scheduled Worker/Workflow exporting a SQL dump to R2 on a cadence; **document and actually perform** a restore into a scratch DB (NFR-B1..B3). **Done when:** a restore from an R2 export reproduces current data and the procedure is written down.
